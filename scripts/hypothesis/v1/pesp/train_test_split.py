...
def merge_csv(scene_index_list):
    ...
    for scene_index in tqdm(scene_index_list):
        scene_item = os.path.join(
            s3d_csv_pth, scene_index, f"hypothesis_v1/{scene_index}_pesp.csv")
...
# 数据目录存储文件夹
os.makedirs(os.path.join(s3d_csv_pth, "hypothesis_v1"), exist_ok=True)
...
output_csv_train = os.path.join(s3d_csv_pth, "hypothesis_v1/train.csv")
...
output_csv_val = os.path.join(s3d_csv_pth, "hypothesis_v1/val.csv")
...
output_csv_test = os.path.join(s3d_csv_pth, "hypothesis_v1/test.csv")
